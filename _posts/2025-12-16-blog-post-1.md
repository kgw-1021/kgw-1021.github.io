---
layout: post
title: "[연구노트]MPPI의 한계를 넘어서: Analytical Koopman과 Safety Projection의 결합"
date: 2025-12-16 14:41:00 +0900
categories: [Control Theory, Robotics]
tags: [MPPI, Koopman Operator, MPC, Safety Critical, Optimization]
author: Geonwoo Kim
math: true
mermaid: true
...
```mermaid
graph TD
    A[State Estimation x_t] --> B[Analytical Koopman Update]
    B -->|Compute A_t, B_t| C{MPPI Loop}
    
    subgraph "Sample Processing (Parallel)"
    D[Sample Generation] --> E[Neural Warm-Start]
    E -->|Initial Guess| F[QP Projection]
    F -->|Constraint: Hz < g| F
    F -->|Valid Sample u*| G[Cost Calculation]
    end
